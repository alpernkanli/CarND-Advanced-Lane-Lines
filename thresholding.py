...
class Thresholder:
    def __init__(self, grad_threshold, dir_threshold, color_threshold, sobel_kernel):
    ...
    def mag_thresh(self, image):
        sobelx = cv2.Sobel(image, cv2.CV_64F, 1, 0, ksize=self.sobel_kernel)
        sobely = cv2.Sobel(image, cv2.CV_64F, 0, 1, ksize=self.sobel_kernel)
    ...
    def color_thresh(self, image):
        color_binary = np.zeros_like(image)
        color_binary[(image > self.color_threshold[0]) & (image < self.color_threshold[1])] = 1
        return color_binary